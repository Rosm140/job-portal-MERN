...
import {
  ...
  SlidersHorizontal, X, Search, MapPin,
} from "lucide-react";
...
const JobsPage = () => {
  ...
  const [localLoc,     setLocalLoc]     = useState(searchParams.get("location") || "");
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    updateParams({ search: localSearch || undefined, location: localLoc || undefined, page: 1 });
  };
      ...
      <div className="bg-white border-b border-gray-200 py-4 px-4 sticky top-16 z-30">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row gap-3 items-center">
          ...
          <form onSubmit={handleSearch}
            className="flex flex-1 max-w-2xl border border-gray-300 rounded-lg overflow-hidden hover:border-blue-400 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
            ...
            <input
              ...
              className="w-28 px-3 py-2 text-sm focus:outline-none bg-white text-gray-800"
            />
            <button type="submit"
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {hasFilters && (
              <button onClick={resetFilters}
                className="flex items-center gap-1 text-xs text-red-500 hover:text-red-700 px-2 transition-colors">
                <X size={12} /> Clear
              </button>
            )}
        ...
        {/* Expandable filters */}
        {filtersOpen && (
          <div className="max-w-7xl mx-auto mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-5">
            <FilterGroup
              label="Job Type" options={JOB_TYPES} value={params.jobType}
              onChange={(v) => updateParams({ jobType: v === params.jobType ? undefined : v, page: 1 })}
            />
            <FilterGroup
              label="Work Mode" options={LOC_TYPES} value={params.locationType}
              onChange={(v) => updateParams({ locationType: v === params.locationType ? undefined : v, page: 1 })}
            />
            <FilterGroup
              label="Experience" options={EXP_LEVELS} value={params.experienceLevel}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={<Button onClick={resetFilters} variant="outline">Clear All Filters</Button>}
          />
        ) : (
          <>
            ...
            {pagination?.pages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-8">
                <button
                  ...
                  className="flex items-center gap-1 px-4 py-2 text-sm border border-gray-300 rounded-lg text-gray-600
                    hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed bg-white transition-colors">
